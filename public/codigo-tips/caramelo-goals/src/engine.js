...
function filterByAccuracy(signals, values, horizon = 3, minAccuracy = 60) {
  if (!signals.length) return { filtered: [], stats: {}, approvedPatterns: [] };

  // Roda backtest em TODOS os sinais brutos primeiro (pra que o Mapa
  // de Sinais tenha stats de qualquer padrão que disparou pelo menos
  // uma vez, mesmo os reprovados)
