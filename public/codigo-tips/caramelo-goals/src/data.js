...
// Timeout default pra fetch (5s — depois disso assume API offline)
...
const FETCH_TIMEOUT_MS = 4000; // 4s — não trava a UI se API estiver lenta

function isCacheValid(entry) {
  return entry && (Date.now() - entry.timestamp) < CACHE_TTL_MS;
}

// ⚠️ Fetch com timeout — se a API demorar mais que FETCH_TIMEOUT_MS, aborta
// (sem isso, o navegador pode esperar indefinidamente travando a UI inteira)
...
async function fetchLive(liga = 'copa') {
  ...
  try {
    const res = await fetchWithTimeout(`${API_BASE_URL}/api/live?liga=${encodeURIComponent(liga)}`, {
...
async function fetchHistory(liga = 'copa') {
  ...
  try {
    const res = await fetchWithTimeout(`${API_BASE_URL}/api/history?liga=${encodeURIComponent(liga)}`, {
      headers: { 'ngrok-skip-browser-warning': 'true' },
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
...
async function fetchOdds(liga = 'super') {
  ...
  try {
    const res = await fetchWithTimeout(`${API_BASE_URL}/api/odds?liga=${encodeURIComponent(liga)}`, {
      headers: { 'ngrok-skip-browser-warning': 'true' },
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
...
async function loadMarket(marketKey = 'copa') {
  ...
  // Tenta buscar da API
  const live = await fetchLive(marketKey);
  ...
  return {
    ...
    power,         // { league_lambda, btts_baseline } ou null
    atualizado,    // ISO timestamp ou null
    fonte,         // 'api' ou 'fallback'
    realCount: live ? live.serie_over25?.length : 0, // quantos pontos são reais (no fim)
  };
}
