...
async function notifyAll(signal) {
  ...
  // Filtros
  if (signal.confidence < cfg.filters.minConfidence) return;
  ...
  if (cfg.telegram.token && cfg.telegram.chatId) {
    sendTelegram(cfg.telegram.token, cfg.telegram.chatId, formatSignalForTelegram(signal));
  }

  // Discord
  if (cfg.discord.webhook) {
    sendDiscord(cfg.discord.webhook, signal);
  }
}
